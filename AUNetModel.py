...
def bilinear_Upsample(input_Tensor, channels):
     ...
     buc = Conv2D(channels, 3, activation = 'relu', padding = 'same', kernel_initializer = 'he_normal')(buc)
     return buc

def dense_Upsample(input_Tensor, channels):
    #CONV 4*CHANNELS => BN => RELU
    conv = Conv2D(4*channels, 3, activation = None, padding = 'same', kernel_initializer = 'he_normal')(input_Tensor)
    conv = Activation('relu')(BatchNormalization()(conv))
    #PIXEL SHUFFLE
...
def Channel_Attention(merge,channels):
    ...
    fc1 = Dense((int)(2*channels/16), activation='relu', kernel_initializer='he_normal')(gap)
    fc2 = Dense(2*channels, activation='sigmoid', kernel_initializer='he_normal')(fc1)
...
def concat(low_level, high_level, channels):
    ...
    #CONV=>BN=>RELU
    added = Conv2D(channels, 3, activation = None, padding = 'same', kernel_initializer = 'he_normal')(added)
